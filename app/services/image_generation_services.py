...
class ImageGenerationService:
    ...
    async def generate_from_sketch(
        ...
        try:
            ...
            # Detectar errores de sobrecarga (503) o cuota (429)
            err_str = str(exc).lower()
            is_overloaded = any(x in err_str for x in ["503", "overloaded", "unavailable", "429", "quota"])
            ...
            # Si no es un error de carga o el fallback también falla
            print(f"[ImageGenerationService] Error: {exc}")
            raise HTTPException(status_code=500, detail="Error en los motores de IA.")
    ...
    def _publish_save_event(self, user_id, workflow_id, material_id, image_bytes):
        try:
            ...
            print(f"[PubSub] Error al encolar guardado: {e}")